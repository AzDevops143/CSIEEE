...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{CSL6010-Cybersecurity Major Project}
\fancyhead[R]{}
\fancyfoot[L]{GitHub Repository: \url{https://github.com/AzDevops143/CSIEEE}}
\fancyfoot[R]{\thepage}
...
\author{\textbf{Team of 7 Members}\\
\small Source Code Repository: \url{https://github.com/AzDevops143/CSIEEE}}
\date{\today}

\begin{document}

\maketitle
\thispagestyle{fancy} % Force header/footer on title page too
...
\node (generator) [module, below=of keyboard] {\textbf{Dataset Generator}\\(\texttt{create\_dataset.py})};
\node (csvdata) [data, below=of generator] {vr\_telemetry\_dataset.csv\\(72 Hz)};

\node (defender) [module, below left=1.5cm and -0.5cm of csvdata] {\textbf{Defender: CASOM}\\(\texttt{defender.py})};
...
\node (result2) [data, fill=blue!10, below=of attacker2] {Inferred Word:\\'dsw' (Failed)};

% Arrows
\draw [arrow] (keyboard) -- node[anchor=west] {Layout Mapping} (generator);
\draw [arrow] (generator) -- node[anchor=west] {Generate Mock CSV} (csvdata);

\draw [arrow] (csvdata) -| node[anchor=south] {Unprotected} (attacker1);
\draw [arrow] (csvdata) -| node[anchor=south] {Protected} (defender);
...
\begin{enumerate}
    ...
    \item \textbf{Data Generator (\texttt{create\_dataset.py})}: To simulate a real user wearing a VR headset, this module takes a target string (e.g., 'foxenter') and generates a highly realistic \texttt{.csv} dataset file. It models the natural human behavior of slowing down head movement when aiming at a key by producing dense clusters of data points (simulating a 72 Hz sampling rate) around the target coordinates, injecting slight micro-variances to mimic natural inaccuracy.
    ...
    \item \textbf{Defender Module / CASOM (\texttt{defender.py})}: Our primary contribution, this module acts as the OS-level middleware. When active, it intercepts the raw gaze points parsed from the dataset. Simulating "Typing Detected" state, it dynamically injects Laplacian noise (with a configurable scale, defaulting to 1.5) into the $(x, y)$ coordinates. This scatters the data points, deliberately degrading the spatial resolution available to background apps.
    
    \item \textbf{Main Orchestrator (\texttt{main.py})}: The overarching script that executes the simulation by parsing the \texttt{vr\_telemetry\_dataset.csv} file row-by-row and routing it through two parallel pipelines:
    \begin{itemize}
